...
router.post('/match', (req, res) => {
  ...
  console.log(
    'Received request to match user:',
    user_id,
    'with offer:',
    job_id
  );
  ...
  if (!user_id || !job_id) {
    ...
    return res
      .status(400)
      .json({ success: false, error: 'Missing user ID or job ID' });
  }
  ...
  pythonProcess.stdout.on('end', () => {
    try {
      ...
      if (!res.headersSent) {
        res.json({
          success: true,
          matchingPercentage: result.matching_percentage,
        });
      }
    } catch (error) {
  ...
  pythonProcess.on('exit', (code) => {
    if (code !== 0 && !res.headersSent) {
      res
        .status(500)
        .json({ success: false, error: 'Python script exited with an error' });
    }
  });
});
